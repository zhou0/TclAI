...
# I messed up with multiple duplicate blocks. Let's fix the whole file by replacing the duplicated methods.
pattern = r'        method CopyAsImage \{txt\} \{.*?        \}(?:\s+\} err\]\} \{.*?        \}){1,2}'
import re
new_content = re.sub(pattern, """        method CopyAsImage {txt} {
            set filetypes [list \\
